...
`P-028` 是当前唯一的 `1624 x 969` 宽窗口舞台参考。其余 `1024 x 1024` 图片多数是展示画布，不能把图片像素直接当成应用 viewport。

原型中的 `OpenNeko` 文案属于 legacy 品牌债务。新界面只使用 NekoPapa；NNA 只用于工程命名，不作为第二个产品标题。
