...
`priority:*`。状态和平台标签按需添加。清单已于 2026-07-18 同步到 GitHub；规范来源
仍以 `.github/labels.yml` 为准。
...
- 证据：2026-07-18 GitHub API 回读 `main.protected=true`；管理员也受保护，禁止
  force push/删除，启用 PR、strict required checks、线性历史和 conversation resolution。
- 范围：治理文件、squash-only 合并、合并后自动删除分支和线上主分支保护均已完成。
- 验收：线上设置与 [GOVERNANCE.md](../GOVERNANCE.md) 一致，5 个 required checks 名称
  与实际 workflow 完全相同。

### CI-001：建立真实可执行的 CI 基线（已完成）

- 类型/区域/优先级：`type:tech-debt`、`area:build-release`、`priority:p1`
- 证据：CI 已在 `ebecd44` 的 `29609447293` 运行中通过 repository hygiene、前端
  check/build、Rust fmt/clippy/test，以及 macOS/Windows Native Stage 构建。
...
- 证据：产品和桌面包已统一为 NekoPapa，但 CMake project/target、sidecar binary、
  Stage namespace 和 Frozen Qt 资源 URI 仍包含 `openneko` legacy 标识。
...
- 证据：已有 macOS/Windows 双平台 Native Stage 构建 CI；仍没有两平台 health/runtime、
  安装包或发布级运行记录。
