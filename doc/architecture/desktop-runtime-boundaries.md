...
4. OpenNeko Engine/NNA Core 最终负责可持久、可同步的同伴领域状态和高层行为意图。
...
OpenNeko/NNA Core -- future explicit render-intent contract -->
...
- `img/` 当前是未索引的产品原型输入，不是运行时资源。目标位置和 manifest 见[项目结构](../OpenNeko%20Engine%20完整项目结构.md)。
