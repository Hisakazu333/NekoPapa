...
| NNA / NekoCore-Nano layer | Stub 模式 | Native Stage 尚未接入完整 NekoCore-Nano 领域状态 |
...
NekoPapa 是当前产品、桌面包和仓库的唯一名称。Nekonano-Aether（NNA）与 NekoCore-Nano 指同一底层体系：NNA 用于 C++ `nna` 命名空间、`NNA_` 构建选项和 `feat/nna-*` 等工程前缀，NekoCore-Nano 是核心名称。当前公开仓库默认只构建 Stub，不包含完整 Core 实现。
